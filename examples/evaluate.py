...
def main(h3ds_path, h3ds_token, output_dir):
    ...
    for scene_id in h3ds_scenes:
        ...
        mesh_gt.save(os.path.join(output_dir, 'full_head',
                                  f'{scene_id}_gt.obj'))
        ...
        mesh_pred_aligned.save(
            os.path.join(output_dir, 'full_head', f'{scene_id}_pred.obj'))
        ...
        mesh_gt.save(
            os.path.join(output_dir, 'face_sphere', f'{scene_id}_gt.obj'))
        ...
        mask_sphere = np.where(
            np.linalg.norm(v_pred -
                           v_pred[landmarks_pred['nose_tip']], axis=-1) < 95)
